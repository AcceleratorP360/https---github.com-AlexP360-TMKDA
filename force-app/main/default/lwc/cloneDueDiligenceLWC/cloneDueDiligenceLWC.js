...
import getDueDiligenceRecrd from '@salesforce/apex/DueDiligenceCloneCtrl.getDueDiligenceRecrd';
import cloneRecords from '@salesforce/apex/DueDiligenceCloneCtrl.cloneRecords';
...
export default class CloneDueDiligenceLWC extends NavigationMixin(LightningElement) {
    ...
    records({
        ...
        if (data) {
            ...
            //this.name = data.Name + ' ' + formattedDate;
             // set to the number of characters you want to keep
            /*var length = 20; 
            var pathname = data.Name;
            alert("Name is " +pathname);
            var trimmedPathname = pathname.substring(0,length);*/
            this.name = data.Name;
            //alert(trimmedPathname);
        ...
        cloneRecords({
            ...
            .then(result => {
                ...
                const evt = new ShowToastEvent({
                    title: 'Success',
                    message: 'Due Diligence has been cloned successfully!',
                    variant: 'Success',
                });
